...
/// Scenario Selection Page - Step 1a
/// Users can choose between AI camera detection or manual scenario selection
...
class _ScenarioSelectionPageState extends State<ScenarioSelectionPage>
  ...
  void initState() {
    super.initState();
    // _initializeCamera(); // CAMERA DISABLED
    _setupAnimations();
  ...
  @override
  void dispose() {
    // _controller?.dispose(); // CAMERA DISABLED
  ...
  // CAMERA FUNCTIONALITY DISABLED FOR GOOGLE PLAY
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // Mode switch button - TEMPORARILY DISABLED FOR GOOGLE PLAY
          /* GestureDetector(
            onTap: () {
              setState(() {
  ...
  */
  ...
  // CAMERA METHOD: _onAIScanContinue - DISABLED
  /*
  void _onAIScanContinue() {
    if (_detectedScene != null && _detectedEquipment.isNotEmpty) {
      // Save AI detection data to Provider
      final provider = context.read<WorkoutGuideProvider>();
      provider.selectScenarioByCode(_detectedScene!);

      // Save detected equipment
      for (var equipmentCode in _detectedEquipment) {
        provider.toggleEquipmentByCode(equipmentCode);
      }

      // Navigate directly to Step 2 (Workout Mode Selection) - AI跳过器材选择
      Navigator.pushNamed(
        context,
  ...
  */
  ...
  void _onSelectionContinuePressed() {
    if (_selectedScenario != null) {
      // Navigate to Equipment Selection (Step 1b) with selected scenario
